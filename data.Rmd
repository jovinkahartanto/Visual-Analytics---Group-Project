...
stop_fin <- read_csv("datasets/stop_fin.csv")%>%select(long,lat,Possible_Location)
...
cc_loyalty<- merge(cc,loyalty, by.x = c("date","price","location"), by.y = c("date","price","location"), all.x = TRUE)%>% select(-c(weekday.x,dayofmonth.x)) 
...
```{r credit card and loyalty card mapping - final cc loyalty}
...
```{r final tagging DIFFERENT RESULTS}
