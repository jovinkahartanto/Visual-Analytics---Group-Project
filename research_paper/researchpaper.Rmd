...
Exploratory data analysis (EDA) with histogram, heatmap and boxplot were used to give users an overview of GAStech employees' credit card and loyalty card transactions over the two weeks. Heatmap is used for a quick summary of the most popular locations based on transaction frequency. This function is meant for high-level visualisation and not detailed analysis. Detailed histogram by location and time period is used to visualise distribution and popularity of locations at different time of the day. Any transactions at odd timing can be isolated and scrutinise in detail. Boxplot shows the median and 25th and 75th percentiles of transaction amounts at each location, and to visualise any anomalies. Bar charts and histograms were chosen since they are easy to grasp without significant statistical knowledge. Interactivity is a significant feature of the EDA charts. Given that this is an exploratory phase, all dataset parameters were included as much as possible to aid the user in discovering insights.

Exploratory spatial data analysis (ESDA) with Abila map and GPS routes were used to visualise and explore island map and determine movements and location of employees' cars at specific time periods. This is complemented by dot plot of visits of employees by time and day at specific locations. This helps users to identify any overlaps in visit location and time of employees to determine any relationships among employees.
...
The application was developed with Shiny, a R tool for generating interactive web apps. As Shiny can receive user input and return output to the user using R code, it allows for interactivity. The input values can be changed at any time, and the result will change as well. Shiny also uses the R packages available on R-cran, which expands on the core R functions and improves the app's usefulness.

The Shiny application is designed according to the flow of the Vast Challenge questions. 
